...
| 25 | **LevelPlay reklam (ilk monetizasyon)** — `com.unity.services.levelplay` 9.5.1 + EDM4A. App Key `27f1c2215`, Interstitial ad unit `gn5krubs30l4gsgf` (LevelPlay console'da açıldı). `AdConfig` SO + `AdManager` (`__Bootstrap`'te), interstitial `LevelFlow.NextLevel`'de **her 2 yanma VE son reklamdan ≥70 sn** (ikisi birden = frequency cap; reflection ile runtime doğrulandı). Editörde init+load doğrulandı. **Kalan (Kaan, cihaz):** Integration Manager adapter'lar + EDM Android Resolve + cihazda Test Suite + payment preferences. Detay: `PlayStore/LEVELPLAY_SETUP.md`. | `0c09d533` |
| 26 | **Yayın hazırlığı (2026-09-25)** — ironSource hesabı **ONAYLANDI**. Play Console **app content 11/11 TAMAM** (Government/Financial/Health beyanları=yok, App category=Game/Casual, contact=`kaanekimoz@gmail.com`+`kaanusta.dev`). Cadence her 2 yanma + ≥70 sn'ye çekildi. 6 commit master'a push'landı. Dashboard artık Release aşamasında (closed-test 3/5). | `e18445f3` +push |
| 27 | **🚀 PRODUCTION'A GÖNDERİLDİ (2026-09-26)** — imzalı AAB build edildi (keystore `.../Downloads/Game-Development/Anahtarlar/user.keystore`, alias `ekimozgames`), **targetSdk 36'ya çekildi** (Google zorunlu kıldı, önce 35'le reddedildi), **versionCode 3** / versionName 1.0.0, 73MB. Production release **tüm dünya (177 ülke)**, Advertising ID beyanı = Yes/Advertising or marketing. Leftover "Patimania Test" closed-test tester listesi submission'dan çıkarıldı (Kaan istedi). **12 değişiklik "Send for review"'a gönderildi → "Changes in review".** managedPublishing OFF → onaylanınca otomatik canlı. **Kalan:** Google review (tipik ≤7 gün) → otomatik yayın; + cihazda LevelPlay reklam testi (adapter+Test Suite) + ironSource payment preferences (gelir için). | `dc511576` +push |
